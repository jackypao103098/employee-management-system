...
const createDemoToken = (id: number, email: string, role: "ADMIN" | "EMPLOYEE") => {
  ...
  return {
    ...
    expiresAt: new Date((nowInSeconds + 8 * 60 * 60) * 1000).toISOString(),
  };
};
...
export const login = async ({
  ...
  const demoToken = createDemoToken(1, email, "ADMIN");
  return response({
    accessToken: demoToken.token,
    tokenType: "Bearer",
